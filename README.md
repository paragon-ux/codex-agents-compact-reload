...
| :--- | :--- | :--- |
| **Model Context Protocol (MCP)** | [Model Context Protocol Specification](https://github.com/modelcontextprotocol/specification) | Standardized JSON-RPC 2.0 stdio tool interface used across Waymark. |
...
| :--- | :--- | :--- | :--- |
| **Tier 1: Active Lifecycle Hook** | Out-of-context process execution on compaction boundary. | **100% deterministic.** Instructions prepended before model turn; zero tokens spent remembering to recover. | **OpenAI Codex** (`SessionStart`), **Google Antigravity** (`PreInvocation.injectSteps`), **Claude Code** (`post_compact`), **Hermes Agent** (`pre_llm_call` shell hook). |
| **Tier 2: MCP Ingestion** | In-band Model Context Protocol primitives. | **High reliability.** Standardized pull via resources/prompts. | **Claude Code**, **Cursor Composer**, **Windsurf**, **Cline**, **Hermes Agent** (`mcp_servers`). |
| **Tier 3: Persistent Directives** | Sticky system instruction files (`CLAUDE.md`, `.cursor/rules/*.mdc`). | **Best-effort.** Instructs model to reload rules as step 1 when prior turn history is rolled. | **Cursor**, **Claude Code**, **Antigravity**, **Hermes Agent** (`AGENTS.md` context-file loading). |
...
- A supported AI coding agent harness (OpenAI Codex, Google Antigravity, Claude Code, Cursor, Hermes Agent, etc.).
...
### Preview installation (Codex):

```sh
node scripts/install.mjs --project my-project=<PROJECT_ROOT> --codex-version <CODEX_VERSION> --dry-run
```

### Install for that project (Codex):

```sh
node scripts/install.mjs --project my-project=<PROJECT_ROOT> --codex-version <CODEX_VERSION>
```

### Preview installation (Hermes Agent):

```sh
node scripts/install.mjs --target hermes --project my-project=<PROJECT_ROOT> --dry-run
```

### Install for that project (Hermes Agent):
...
- **[Hermes Agent (`harnesses/hermes/`)](harnesses/hermes/README.md):** `pre_llm_call` shell hook in `~/.hermes/config.yaml` with a compaction-marker gate, first-use consent, and native MCP pairing for the Waymark ledger.
...
- multi-harness payload auto-detection (Codex, Antigravity, Hermes, Markdown, JSON);
- Hermes `pre_llm_call` compaction gating (injects on summary/continuation/fallback/merged markers with no live user turn; no-op on live user messages and ordinary turns);
- Hermes installer line-surgery merge (comment/section preservation, idempotence, CRLF, fail-closed on unsupported shapes);
- Hermes uninstaller scoped entry removal (foreign-entry preservation, emptied-key removal, backup reporting);
...
│   └── hermes/                # Hermes Agent integration details
├── scripts/                   # Management scripts
│   ├── install.mjs            # Idempotent harness hook installer (Codex + Hermes)
│   └── uninstall.mjs          # Scoped uninstaller and cleaner
├── src/                       # Core runtime
│   └── reload-agents.mjs      # Universal multi-harness compact bootloader
└── test/                      # Node.js native test suite (24 automated tests)
    ...
    └── hermes.test.mjs        # Hermes shell-hook, installer, and uninstaller tests
```
...
- **[Support Tiers Guide](docs/SUPPORT-TIERS.md)**: 3-tier classification across Codex, Antigravity, Claude Code, Cursor, and Hermes Agent.
- **[Harness Integration Recipes](harnesses/README.md)**: Detailed recipes for Codex, Claude Code, Antigravity, Cursor, and Hermes Agent.
